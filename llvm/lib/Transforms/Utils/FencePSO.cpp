...
void TraverseBBGraphPSO(BasicBlock &BB, AtomicOrdering order, Instruction *lastMemOp) {
  if (BB.empty()) {
  ...
  for (Instruction &I : BB) {
    ...
    // Handle Store instructions
      ...
      if (isa<StoreInst>(lastMemOp)) {
        llvm::errs() << "    Previous OP is a Store, not inserting a fence as PSO allows Store-Store reordering.\n";
        IRBuilder<> Builder(&I);
...
PreservedAnalyses FencePSOPass::run(Module &M,
